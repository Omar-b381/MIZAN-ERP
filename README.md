...
> **"نظام تجاري محلي، معياري، مشفر بمفاتيح Ed25519، فائق السرعة، ومصمم خصيصًا لتلبية متطلبات الشركات والمؤسسات بدقة رياضية ومحاسبية مطلقة مع إمكانات طباعة وتقارير وتصدير فوري للبيانات."**
...
4. [مخططات دورات العمل المتكاملة (Business Cycles & Diagrams)](#-مخططات-دورات-العمل-المتكاملة-business-cycles--workflow-diagrams)
   ...
   - [4.7 دورة طباعة المستندات وتوليد التقارير وتصدير البيانات (Reporting & Export Flow)](#47-دورة-طباعة-المستندات-وتوليد-التقارير-وتصدير-البيانات-reporting--export-engine)
...
يهدف المشروع إلى توفير بديل قوي، آمن، وخفيف الوزن لأنظمة ERP السحابية البطيئة والمكلفة، مع التركيز على **الخصوصية التامة للبيانات (Local-First)**، ودعم كامل للغة العربية والاتجاه من اليمين لليسار (RTL-First)، والتوافق الكامل مع متطلبات السوق الإقليمي والمصري (الجنيه المصري، ضريبة القيمة المضافة 14%، دليل الحسابات الموحد، تفقيط الأرقام، وتوليد تقارير Excel و PDF أصلية).
...
5. **ضعف التوطين الإقليمي وأدوات الطباعة والتصدير**: عدم جاهزية معظم الأنظمة لطباعة المستندات والفواتير بترويسة رسمية، أو تصدير التقارير بتنسيق Excel RTL متوافق عربياً.
...
graph TD
    ...
    Engine --> Auth["Auth & Multi-Company RBAC Matrix (Argon2id)"]
    Engine --> License["Ed25519 Cryptographic Licensing & Trial Engine"]
    ...
    Engine --> Reports["Phase 9: Comprehensive Reports Suite"]
    Engine --> ExportEngine["Phase 9: Native RTL Excel & ZIP Export Engine"]
    Engine --> DB[("SQLite Database (WAL Mode / ACID Compliant / Auto-Backup)")]
```
...
يوضح المخطط التالي كيف تترابط كافة وحدات النظام (المبيعات، المشتريات، المخازن، الحسابات، الموارد البشرية، والتقارير) في تدفق متجانس ومحكم رياضياً:
...
flowchart TD
    ...
    subgraph Reporting_Export ["منظومة الطباعة والتقارير والتصدير (Phase 9)"]
        REP["مركز التقارير المالية والتشغيلية"]
        PRINT_ENGINE["محرك طباعة الفواتير A4 مع التفقيط"]
        XLSX["مولد إكسيل الأصلي RTL Excel"]
        ZIP["محرك الأرشيف المجمع ZIP"]
        
        TB & Finance & Inventory & Sales & Procurement --> REP
        REP --> XLSX
        INV & BILL & SO & PO --> PRINT_ENGINE
        INV & BILL --> ZIP
    end
```
...
stateDiagram-v2
    ...
    state Customer_Invoice {
        ...
        Post_Invoice --> Print_Preview: معاينة وطباعة المستند A4 مع التفقيط
        Post_Invoice --> Export_XLSX: تصدير الفاتورة إلى إكسيل
    }
...
sequenceDiagram
    ...
    System->>GL: تسجيل القيد: Debit Purchases/Expense + Debit VAT Input, Credit A/P
...
flowchart TD
    ...
    subgraph Reporting ["التقارير والمخرجات المالية (Phase 9)"]
        TB["ميزان المراجعة المفلتر (Trial Balance)"]
        IS["قائمة الدخل والأرباح والخسائر (P&L)"]
        GL_Det["دفتر الأستاذ العام التفصيلي (General Ledger)"]
        Excel["تصدير Excel أصلي RTL"]
    end
    ...
    Assets & Liabilities & Equity & Revenue & Expenses --> TB & IS & GL_Det
    TB & IS & GL_Det --> Excel
```
...
### 4.7 دورة طباعة المستندات وتوليد التقارير وتصدير البيانات (Reporting & Export Engine)

يوضح هذا المخطط تدفق استخراج البيانات وتوليد التقارير والمستندات الورقية والإلكترونية في المرحلة 9:

```mermaid
flowchart TD
    subgraph Core_Data ["قاعدة البيانات والمعاملات"]
        Invoices["فواتير المبيعات والمشتريات"]
        Moves["قيود اليومية ودفتر الأستاذ"]
        Partners["حسابات العملاء والموردين"]
        Stock["أرصدة وحركات المستودعات"]
    end

    subgraph Processing_Engine ["محرك المعالجة والتحويل (Rust & TS)"]
        Tafqeet["محرك التفقيط المالي (Currency in Words)"]
        Report_Agg["مجمع التقارير (Reports Aggregator)"]
        XlsxWriter["مولد الإكسيل الأصلي (rust_xlsxwriter RTL)"]
        ZipArchiver["ضاغط الحزم المجمعة (ZIP Archiver)"]
    end

    subgraph Business_Outputs ["المخرجات الرسمية للأعمال"]
        A4_Print["طباعة المستندات الرسمية (A4 Letterhead + QR)"]
        Reports_UI["واجهة استعراض ومقارنة التقارير التفاعلية"]
        Excel_File["ملفات جداول إكسيل (.xlsx) من اليمين لليسار"]
        Zip_Batch["أرشيف المستندات المجمعة (.zip)"]
    end

    Invoices --> Tafqeet --> A4_Print
    Invoices --> ZipArchiver --> Zip_Batch
    Moves & Partners & Stock --> Report_Agg --> Reports_UI
    Report_Agg --> XlsxWriter --> Excel_File
    Reports_UI --> A4_Print
```
...
- **المصادقة والأدوار (Authentication & RBAC)**: تشفير آمن لمصفوفة صلاحيات تفصيلية (Permissions Matrix).
- **دليل جهات الاتصال الموحد (Unified Partners Directory)**: سجل موحد للعملاء والموردين.
- **سجل النشاط والمحادثات (Chatter & Audit Trail)**: توثيق غير قابل للحذف للعمليات.

### المرحلة 2: الكتالوج والمخزون المزدوج (Phase 2 — Products & Inventory) ✅
- **كتالوج المنتجات ووحدات القياس (UOMs)**: شجرة تصنيفات، نسب التحويل، والتتبع بأرقام Lot و Serial.
- **محرك المخزون بالقيد المزدوج (Double-Entry Stock Engine)**: أذون الاستلام (`WH/IN/`)، أذون الصرف والتسليم (`WH/OUT/`)، والتحويلات الداخلية (`WH/INT/`).
- **الجرد الفعلي وتسوية الفروقات (Physical Inventory Adjustments)**: جلسات جرد دورية مع توليد قيود التسوية.

### المرحلة 3: محرك إدارة المبيعات (Phase 3 — Sales Engine) ✅
- دورة حياة عروض الأسعار وأوامر البيع الكاملة.
- التسعير والخصومات والضريبة 14%.
- الربط التلقائي بالمخازن (Sales $\rightarrow$ Delivery Trigger) لتوليد أذون الصرف فور الاعتماد.

### المرحلة 4: محرك المشتريات والموردين (Phase 4 — Purchases Engine) ✅
- دورة أوامر الشراء وطلبات الأسعار (RFQs & POs).
- الربط التلقائي بالاستلام المخزني (Purchases $\rightarrow$ Receipt Trigger) لتوليد أذون الاستلام الوارد فور الاعتماد.

### المرحلة 5: الحسابات العامة، الفواتير والمدفوعات (Phase 5 — General Ledger & Accounting) ✅
- **دليل الحسابات المصري الموحد (Egyptian Standard COA)** ودفاتر اليومية المتخصصة.
- **محرك القيود المزدوجة المتوازنة**: حتمية التوازن الصارمة `SUM(debit) == SUM(credit)`.
- فواتير المبيعات والموردين وسندات القبض والصرف مع التسوية المباشرة.

### المرحلة 6: الموارد البشرية وإدارة الموظفين (Phase 6 — Human Resources) ✅
- دليل الموظفين والهيكل الإداري والأقسام والوظائف.
- عقود العمل والرواتب بالأعداد الصحيحة (`wage_cents`).
- إدارة طلبات الإجازات والعطلات وسجلات الحضور والانصراف مع احتساب ساعات العمل الفعلي.

### المرحلة 7: لوحة المؤشرات التنفيذية وحزم التثبيت (Phase 7 — Dashboard & Release Packaging) ✅
- لوحة القيادة التنفيذية اللحظية (Executive Dashboard) مع المؤشرات التشغيلية والمالية وأشرطة التنبيهات.
- جاهزية حزم التثبيت والإنتاج لنظام Windows (`.msi / .exe`).

### المرحلة 8: التقسية والتغليف والترخيص التجاري (Phase 8 — Production Hardening & Licensing) ✅
- **تشفير كلمات المرور عبر Argon2id** مع ترقية شفافة وتلقائية لكلمات المرور القديمة.
- **الترخيص الرقمي المستقل (Offline Ed25519 Cryptographic Licensing)** بدون الحاجة للاتصال بالإنترنت.
- **فترة تجريبية تقييمية (7-Day Trial Mode)** مع بصمة عتاد الجهاز (Machine ID).
- **النسخ الاحتياطي الذري الآمن (Online Hot Backup Engine)** لقواعد بيانات SQLite.
- **حماية التشغيل الأحادي (Single-Instance Mutex Lock)** ومنظومة تسجيل التشخيصات وتصدير سجلات الأخطاء.

### المرحلة 9: الطباعة والتقارير وتصدير البيانات (Phase 9 — Printing, Reports & Data Export) ✅
- **محرك قالب PDF المباشر (Direct PDF Template Engine)**:
  - يبني مستند A4 كاملاً في الذاكرة (HTML → html2canvas → jsPDF) دون فتح واجهة المتصفح أو نوافذ الطباعة.
  - القالب يتضمن: ترويسة الشركة الكاملة، بيانات العميل/المورد مع الرقم الضريبي، جدول البنود الملون، التفقيط المالي بالعربي، وخانات التوقيع والاعتماد.
  - يُحفظ مباشرة كـ `.pdf` بضغطة زر واحدة من جدول الفواتير أو نافذة المعاينة.
- **طباعة المستندات مع الترويسة المعتمدة (Hardware Print Command)**:
  - أمر طباعة مباشر للطابعة المتصلة عبر `@media print` بحجم A4 صارم.
  - إخفاء تلقائي لكل عناصر الواجهة غير المتعلقة بالمستند.
- **حزمة التقارير الشاملة المدمجة (Reports Suite)**:
  - **التقارير المالية**: ميزان المراجعة التجريبي، قائمة الدخل والأرباح والخسائر، ودفتر الأستاذ العام بأرصدة تراكمية مستمرة.
  - **تقارير المبيعات والعملاء**: تحليل المبيعات التفصيلي، كشف حساب عميل مفصل، وأعمار ديون العملاء (0-30، 31-60، 61-90، 90+ يوم).
  - **تقارير المشتريات والموردين**: تحليل المشتريات، كشف حساب مورد، وأعمار ديون الموردين.
  - **تقارير المخزون**: جرد المخزون وتقييم بضاعة آخر المدة، سجل حركة المخزون، وتقارير نواقص المخزون ونقاط إعادة الطلب.
  - تصدير Excel مباشر (.xlsx) من شاشة أرصدة المخزون وكتالوج الأصناف.
  - مراعاة كاملة لحالة الوحدات المرخصة (Module Licensing Gating).
- **محركات التصدير الأصلية فائقة السرعة (Native Rust Export Engines)**:
  - تصدير مباشر إلى ملفات Excel (.xlsx) عبر مكتبة `rust_xlsxwriter` مع اتجاه كامل من اليمين لليسار (RTL-First)، تثبيت صفوف العناوين، وتنسيق الأرقام والعملات من الوحدات الدقيقة (`cents`).
  - محرك تصدير مجمع للفواتير والمستندات كأرشيف مضغوط (.zip).
...
### 3. التصدير العربي الأصلي والتفقيط دون مكتبات سحابية ثقيلة
* **التحدي**: إنتاج ملفات Excel وتقارير وفواتير قابلة للطباعة محلياً تدعم اللغة العربية والاتجاه من اليمين لليسار RTL بالكامل على كافة الأجهزة الضعيفة والمتوسطة دون متطلبات تشغيل خارجية.
* **الحل**: كتابة محرك تصدير Rust أصلي يعتمد على `rust_xlsxwriter` لإنتاج ملفات ثنائية مضغوطة وفائقة السرعة، وبناء واجهة طباعة A4 نظيفة مع محرك تفقيط مالي باللغة العربية.
...
3. **تشغيل الاختبارات التلقائية الشاملة (Run All 75 Tests)**:
   - اختبارات الواجهة الأمامية (Vitest - 36 اختبار عبر 10 ملفات):
     ```bash
     npm test
     ```
   - فحص واختبارات المحرك الخلفي (Rust Integration Tests - 39 اختبار):
     ```bash
     cd src-tauri
...
5. **بناء حزمة الإنتاج المستقلة لنظام Windows (Build Production Executable)**:
   ...
   npm run build
   cargo build --release
   cargo build --release --bin mizan-license-generator
   ```

6. **التشغيل المحمول (Portable Release Bundle)**:

   بعد البناء، يمكن تشغيل التطبيق مباشرة من مجلد `release-bundle/` الذي يحتوي على:

   | الملف | الوصف |
   | :--- | :--- |
   | `mizan-erp.exe` | التطبيق الرئيسي — نظام ميزان ERP |
   | `mizan-license-generator.exe` | أداة توليد التراخيص التجارية |
   | `WebView2Loader.dll` | مكتبة WebView2 المطلوبة للتشغيل على Windows |

   > **⚠️ مهم:** يجب الإبقاء على الملفات الثلاثة في نفس المجلد دائماً لضمان التشغيل الصحيح. نقل `mizan-erp.exe` بمفرده سيؤدي لخطأ `WebView2Loader.dll was not found`.
...
| **Phase 9** | الطباعة والتقارير وتصدير البيانات | ✅ مكتمل | طباعة A4، تفقيط مالي، ميزان مراجعة، أرباح وخسائر، وتصدير Excel RTL و ZIP |
...
2. **شركات التوزيع والتوريدات**: التي تتطلب تتبعًا دقيقًا للمخازن المتعددة، أذون الاستلام والصرف، وتسوية الفواتير وطباعتها للعملاء فورياً.
3. **المحاسبين والمديرين الماليين**: الراغبين في أداة دقيقة تعمل محليًا دون انقطاع، تضمن توازن القيود اليومية وميزان المراجعة وتوفر تصدير مباشر إلى Excel.
